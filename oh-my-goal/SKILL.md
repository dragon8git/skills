...
Treat the accepted checklist as the execution contract. Implement only the confirmed scope. Continue until you encounter work that you cannot resolve independently or that requires the user's intervention: `直到遇到你无法独立解决或需要我介入的工作为止。` Run the real project or task-appropriate validation, verify every applicable checklist item, and report the evidence plus all blocked or unverified items. Do not invoke or create the system `/goal` command; this skill only supplies the plan and acceptance contract.
