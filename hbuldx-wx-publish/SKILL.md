...
Use `/Applications/HBuilderX.app/Contents/MacOS/cli publish mp-weixin` for a user-authorized release. Uploading changes the WeChat platform; do not upload unless the user explicitly asks to publish, release, deploy, or upload.
...
6. Confirm the CLI syntax locally with `cli publish mp-weixin --help`, then publish with `--upload true`, the bumped version, appid, key path, description, and robot number (default `1`).
