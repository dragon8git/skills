...
6. Skip non-standard notes. Do not normalize them here. If most files are non-standard, stop and use `$issues2okf` first.
7. Use one independent subagent per eligible issue:
...
8. Have the main agent coordinate, not implement the issue work directly:
   - launch the subagent
...
9. Before a new development batch, perform any repo-required backup commit or checkpoint step if `AGENTS.md` asks for one.
10. After each issue:
...
11. If subagent capability is unavailable in the current harness, stop and report that this skill cannot execute as designed instead of silently falling back to single-agent implementation.
12. Do not silently rewrite issue content, filenames, or frontmatter shape here except for the intentional issue-log updates above: advancing `status`, updating `links`, and appending the next empty review template.
13. Do not perform repository repair or issue inventory reconciliation here:
